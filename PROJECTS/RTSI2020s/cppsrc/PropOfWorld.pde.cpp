...
class PropertyBase {
  ...
  void  reset(){
	...
	}//May but not must change values
};

class MultProperty : public PropertyBase {
  public:
  ///INFO: 
  String getName(){
...
class SinMultProperty : public PropertyBase {
  public:
  ///INFO: 
  String getName(){
	 return "Sinus multiplicative";
	}
  float get0D(){
	return 1;
	}
...
class CosMultProperty : public PropertyBase {
  public:
  ///INFO: 
  String getName(){
	 return "Cosinus multiplicative";
	}
  float get0D(){
	return 1;
	}
...
class DefPerlinProperty : public PropertyBase {
  ...
  float noffs;
  String getName(){
	 return String("Perlin noise {")+ndet+String(",")+noffs+String("}[")+multx+String(",")+multy+String(",")+offsx+String(",")+offsy+ String("]");
	}
  DefPerlinProperty(float mx,float my){
	 multx=mx;multy=my;offsx=offsy=0;noffs=NOISE_OFFS;ndet=NOISE_DETAILS;noiseDetail(NOISE_DETAILS,NOISE_OFFS);
	}
  DefPerlinProperty(float mx,float my,float offx,float offy){
	 multx=mx;multy=my;offsx=offx;offsy=offy;noffs=NOISE_OFFS;ndet=NOISE_DETAILS;noiseDetail(NOISE_DETAILS,NOISE_OFFS);
	}
  ...
  float get1D(float x){
	return noise((offsx+x)*multx);
	}
  float get2D(float x,float y){
	return noise((offsx+x)*multx,(offsy+y)*multy);
	}// MODEL IMPORTANT PART!
  float getMin(){
	 return -0.0;
	}
  float getMax(){
