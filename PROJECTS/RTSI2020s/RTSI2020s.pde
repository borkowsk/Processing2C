...
boolean TRACE_ACTIONS=false;  /// If you want to see each particular action
...
boolean REMEMBER_REALITY_FACTS=false;  /// Is reality fact remembered for opinion or not (ESPECIALLY NOT IN PREDICTION CHECK MODEL!)

float   OWN_MEASURE_PROBABILITY=0.0;   /// "S" - How offten agent use own sensor
boolean SELF_LINKED=(OWN_MEASURE_PROBABILITY==0);  /// Alternatywnie używamy linku do własnego sensora jak do obcego
...
//=======================================
//boolean SCREEN_FULL=true;//To nie działa bo "size() and fullScreen() cannot be used in the same sketch"
...
boolean VIS_AGENTS=false;    ///
float   AGENT_STROKE=2;      ///Stroke width for Agents
boolean WITH_STROKE=true;    ///

boolean WITH_MIN_MAX=false;  /// Wizualizacja minimalnej i maksymalnej opini
boolean WITH_ERRORS=true;    /// Czy jest wykres średniego błędu?
boolean ZERO_ONE_SCALE=false;///Dodanie zera i jedynki do wspólnych maksimów i minimów
boolean visualiseReliabilities=true;  ///
int     FROM_END=(MACRO_STEP*10)/STEPSperSTAT;  ///How many MACRO_STEPS are visible on timelines?
boolean WITH_VIDEO=false;    /// visible autside this file!
...
// MAIN OBJECT
//========================================================
World theWorld=null;//Created & initialised inside setup()
...
void setup()
{
  ...
  println("\nREQUIRED MINIMAL SIZE OF PAINTING AREA IS "+(cwidth*SIDE)*2+"x"+(cwidth*SIDE+STATUSHEIGH));
  ...
  //noiseSeed(10);
  //randomSeed(10);
                                                                          assert MACRO_STEP>1:"MACRO_STEP=="+MACRO_STEP;
                                                                          assert FROM_END>1:"FROM_END=="+FROM_END;
  ...
  //println("\nCURRENT SIZE OF PAINTING AREA IS "+width+"x"+height);//-myMenu.bounds.height???
