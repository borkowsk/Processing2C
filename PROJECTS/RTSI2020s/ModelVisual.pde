...
boolean requestClean=false;  ///visible autside this file!
...
void writeStatusLine()
{
  ...
  textAlign(LEFT, TOP);
  text("Opinions ["+nf(minOpinion)+"..(mean:"+nf(meanOpinion,1,5)
       +" stddev:"+nf(stddOpinion)+").."+nf(maxOpinion)+"]",
          0,WORLD_VIS_SIDE);
  fill(200,0,0);        
  text("mnError:"+meanError+" maxInfo:"+Info_maxInfos+" maxKnow.:"+Info_maxKnowl+" mnSens.:"
       +nf(meanSensor,1,5)+"(N:"+UsedSensors+") mnReliab:"+nf(meanSRelia,1,5),
          0,WORLD_VIS_SIDE+FONT_HEIGHT);
  fill(255,0,255);        
  if(VIS_NETWORK)
    text(" Visible links:"+linkCounter+" Link tres.:"+linkWeightTresh,
          0,WORLD_VIS_SIDE+FONT_HEIGHT*2);//For link filtering
  fill(0,0,200);
  text(" "+theWorld.StepCounter+") Life:"+liveCount+" Fps.:"+frameRate,             //+" Prop.:"+WhichProperty, - in simple_model only ONE PROPERTY
  ...
  if(selected!=null)
  {
      String inspect="Cell "+selectedX+"x"+selectedY+" belong to "+selected;
...
{                                                                                   //print("VI: ");//DEBUG
  ...
  text(""+pow(10,expo)*STEPSperSTAT,width/2+2+tick_sizeH,TIMELINES_HIGTH);
  
  Range commonMinMax=new Range("commonMinMax");
  if(ZERO_ONE_SCALE)
  {
...
void visualizeProperties1D(PropertyBase[] props)
{
  noStroke();
  rectMode(CENTER);  // Set rectMode to CENTER
  PropertyBase property=props[WhichProperty];
...
void visualizeProperties2D(PropertyBase[] props)
{
  noStroke();
  rectMode(CENTER);                                                                //Set rectMode to CENTER
  PropertyBase property=props[WhichProperty];
...
void pixelizeProperties2D(PropertyBase[] props)
{
  strokeWeight(1);
  PropertyBase property=props[WhichProperty];
  float min=property.getMin();
