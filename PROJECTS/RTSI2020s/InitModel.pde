...
class RTSILinkFactory extends LinkFactory {
  ///INFO: 
...
void resetAgents(Agent[][] agents)// MODEL IMPORTANT PART!
{
...
*/
void initializeAgents(Agent[][] agents,// MODEL IMPORTANT PART!
                      World     world)//Agents may need access into properties of the World, but not in simple_model
{
  int side=agents.length;
  for(int i=0;i<side;i++)
    for(int j=0;j<side;j++)
...
//OR
void initializeAgents(Agent[] agents,final World world)//Agents need access into properties of the World
{
  ...
  if(random(1)<AGENTS_DENSITY)
  {
    ...
    //for (int k = 0; k < tmp.sensors.length; k++)//Initial knowledge
    {
         ...
         //int index=(int)random(nrules);
         //float value=world.rules[k].get(index);
         //tmp.issues[k].rules.add(new Info(value,1,0,null));//Full assesment, time 0, source link unknown/autside the world.
    }
