...
class Info { // MODEL IMPORTANT PART!
  ...
  RTSILink sourceOfInfo;//You have to know from whom was the info obtained 
                        //May be NULL!!!
                        
  Info(float ival,float asses,int timem,RTSILink sour)
  {
...
float getAssesedMeanOfSource(ArrayList<Info> infos,Node source)     //MODEL IMPORTANT PART!
...
void checkCoherenceWithReality(float reality,ArrayList<Info> infos,Agent self)//very mild version - MODEL IMPORTANT PART!
{
    for(Info inf:infos)
    {                                     
      if(inf.sourceOfInfo!=null)//When link is available to change
      {
...
float checkFactReliability(Info fact,ArrayList<Info> infos,Agent self)// MODEL IMPORTANT PART!!!
{                                                
    ...
    if(val!=INF_NOT_EXIST)
    {
      ...
      if(self!=null                                                           //only if self is known 
